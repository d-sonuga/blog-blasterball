...
pub fn send_event(event_info: EventInfo) {
    EVENT_HOOKER.lock().as_mut().unwrap().send_event(event_info);
}

pub fn hook_event(event_kind: EventKind, func: BoxedFn<'static>) -> HandlerId {
    EVENT_HOOKER.lock().as_mut().unwrap().hook_event(event_kind, func)
}
...
pub struct EventHooker<'a> {
    ...
    keyboard_handlers: Vec<'a, Handler<'a>>,
