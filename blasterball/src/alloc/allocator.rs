...
// boxes and vectors
static ALLOCATOR: Mutex<LinkedListAllocator> = Mutex::new(LinkedListAllocator {
    head: {
...
mod tests {
    ...
    fn test_allocate_3Kib() {
        unsafe { 
            ...
            let first_region = (*allocator.lock().head).next.unwrap();
    ...
    unsafe fn setup_3_4_2_dealloc_test1() -> (Mutex<LinkedListAllocator>, *mut u8) {
        let mut allocator = new_allocator(10 * ONE_KIB);
        let ptr_3 = allocator.alloc(3 * ONE_KIB, 1);
    ...
    unsafe fn setup_3_4_2_dealloc_test2() -> (Mutex<LinkedListAllocator>, *mut u8) {
        let mut allocator = new_allocator(10 * ONE_KIB);
    ...
    unsafe fn setup_3_4_2_dealloc_test3() -> (Mutex<LinkedListAllocator>, *mut u8) {
        let mut allocator = new_allocator(10 * ONE_KIB);
        let ptr_3 = allocator.alloc(3 * ONE_KIB, 1).unwrap();
        let ptr_4 = allocator.alloc(4 * ONE_KIB, 1).unwrap();
    ...
    fn new_allocator(memory_size: usize) -> Mutex<LinkedListAllocator> {
