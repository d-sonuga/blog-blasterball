...
impl<'a, T: Clone> Vec<'a, T> {
    ...
    pub fn with_capacity(capacity: usize, allocator: &'a dyn Allocator) -> Vec<'a, T> {
...
impl<'a, 'b, T: PartialEq + Clone> PartialEq<Vec<'a, T>> for Vec<'b, T> {
...
impl<'a, T: fmt::Debug + Clone> fmt::Debug for Vec<'a, T> {
